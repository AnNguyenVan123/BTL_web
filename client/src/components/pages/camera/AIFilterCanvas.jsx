...
const AIFilterCanvas = forwardRef(({ videoRef, filter, stickers, brightness, contrast, blending }, ref) => {
  ...
  const propsRef = useRef({ filter, stickers, brightness, contrast, blending });
    ...
    const initMediaPipe = async () => {
      try {
        ...
        faceMesh.onResults((results) => {
          ...
          const { filter, stickers, brightness, contrast, blending } = propsRef.current;
          ...
          ctx.filter = `${baseFilter} brightness(${brightness}%) contrast(${contrast}%)`.trim();
            ...
            results.multiFaceLandmarks.forEach((landmarks) => {
              ...
              stickers.forEach((st) => {
                ...
                if (img && img.complete) {
                    ...
                    const stickerX = centerForehead.x * canvas.width - stickerWidth / 2;
                    const stickerY = centerForehead.y * canvas.height - stickerHeight / 2;
  ...
  return (
    <div className="flex justify-center w-full mt-4 mb-4 flex-grow">
      <canvas 
        ref={ref}
